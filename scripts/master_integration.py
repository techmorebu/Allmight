#!/usr/bin/env python3
"""
Master Integration V3 - Enhanced Pool Scanning + 3-Channel Discord
...
class AllmightScannerV3:
    """Enhanced scanner with better pool extraction"""
    ...
    def extract_all_markets(self, data: Dict) -> Tuple[List[Dict], float]:
        ...
        if 'prices' in uniswap_data:
            for price_data in uniswap_data['prices']:
                ...
                markets.append({
                    'name': f"Uniswap {pair}",
                    'spread_bps': 22,  # Using CoinGecko, spread is minimal
...
def main():
    parser = argparse.ArgumentParser(description='Allmight Scanner V3')
