    ...
    if (block && !isNaN(parseInt(block))) {
      try {
        ...
        await hre.network.provider.send('hardhat_reset', [{
          forking: { jsonRpcUrl: rpcUrl, blockNumber: parseInt(block) },
        }]);
        ...
        await executor.waitForDeployment();
        // Re-fund: fork reset wipes all state — executor needs USDC buffer for Aave premium
        try {
          await ethers.provider.send('hardhat_setBalance', [ADDR.usdcWhale, '0x56BC75E2D63100000']);
          await ethers.provider.send('hardhat_impersonateAccount', [ADDR.usdcWhale]);
          const whaleSig2 = await ethers.provider.getSigner(ADDR.usdcWhale);
          const USDC2 = new ethers.Contract(
            ADDR.USDC,
            ['function transfer(address,uint256) returns(bool)'],
            whaleSig2
          );
          await USDC2.transfer(await executor.getAddress(), BigInt(50e6)); // $50 USDC
          await ethers.provider.send('hardhat_stopImpersonatingAccount', [ADDR.usdcWhale]);
          fundedCount++;
        } catch (fundErr) {
          unfundedCount++;
          // fail-soft — signal may still pass if profit margin absorbs premium
        }
      } catch (resetErr) {
        ...
        results.push({
          ...
          resetError  : resetErr.message?.slice(0, 80),
          targetBlock : parseInt(block),
        });
        revertCounts['FORK_RESET_FAILED'] = (revertCounts['FORK_RESET_FAILED'] ?? 0) + 1;
        console.log(`  ⚠️  fork reset failed — signal=${signal.signalId} block=${block}: ${resetErr.message?.slice(0,50)}`);
        continue;
      }
    }
