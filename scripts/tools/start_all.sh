...
if [[ -f "$REPO/scripts/analysis/arb_volatility_monitor.js" ]]; then
  ...
  (
    ...
    while true; do
      ATTEMPT=$((ATTEMPT+1))
        # S15R7: without --log, arb_volatility_monitor's appendLog() is a no-op
        # (arb_volatility_monitor.js:103 `if (!filePath) return;`) so NO
        # volatility_scan record is ever written — the monitor stays [OK] while
        # producing nothing. The redirect below is the PROCESS LOG (node stdout/
        # stderr plus this wrapper's own echo lines); it is not a record file and
        # must not become one. $REPO-anchored: the wrapper runs in a subshell
        # whose cwd is asserted nowhere.
      node "$REPO/scripts/analysis/arb_volatility_monitor.js" \
          --log "$REPO/logs/volatility_arbitrum.jsonl" \
