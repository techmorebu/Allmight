...
if [[ "${1:-}" == "upload" ]]; then
  ...
  for f in \
    ...
    shadow_execution_ledger.jsonl \
    shadow_execution_totals.json \
    shadow_engine.log \
    execution_gate.json; do
...
if [[ "${1:-}" == "status" ]]; then
  ...
  echo "  ─────────────────────────────────────────────"
  for name in fetcher monitor heat activator watchdog notifier shadow_engine; do
...
" 2>/dev/null || echo "?")
  ...
  # ── Final shadow execution pass (analytics only) ─────────────────────────────
  if [[ -n "$SESSION" && -d "$SESSION_DIR" && -f "scripts/execution/shadow_execution_engine.js" ]]; then
    node scripts/execution/shadow_execution_engine.js \
      --session "$SESSION_DIR" >> "$SESSION_DIR/shadow_engine.log" 2>&1 \
      && log "  ✓ shadow_execution_engine    → session_${SESSION}/shadow_execution_totals.json" \
      || log "  ✗ shadow_execution_engine failed"
  fi
...
# ── Process 7: Shadow execution engine loop ───────────────────────────────────
# Reads live session candidates and writes:
#   shadow_execution_ledger.jsonl
#   shadow_execution_totals.json
# Analytics only — no private key, no deploy, no broadcast.
if [[ -f "scripts/execution/shadow_execution_engine.js" ]]; then
  nohup bash -c "
    while true; do
      node scripts/execution/shadow_execution_engine.js \
        --session '$SESSION_DIR' \
        >> '$SESSION_DIR/shadow_engine.log' 2>&1 || true
      sleep 300
    done
  " >> "$SESSION_DIR/shadow_engine.log" 2>&1 &
  SHADOW_PID=$!
  disown $SHADOW_PID 2>/dev/null || true
  echo "shadow_engine=$SHADOW_PID" >> "$PID_FILE"
  log "✓ Shadow engine    (pid $SHADOW_PID) → session_${SESSION}/shadow_execution_totals.json every 300s"

  # Seed shadow totals immediately once at startup.
  node scripts/execution/shadow_execution_engine.js \
    --session "$SESSION_DIR" >> "$SESSION_DIR/shadow_engine.log" 2>&1 || true
else
  SHADOW_PID="not_installed"
  log "⚠ Shadow engine not found — skipping Process 7"
fi

echo ""
log "Session $SESSION running. PIDs: fetcher=$FETCHER_PID monitor=$MONITOR_PID heat=$HEAT_PID activator=$ACTIVATOR_PID watchdog=$WATCHDOG_PID notifier=$NOTIFIER_PID shadow_engine=$SHADOW_PID"
echo ""
