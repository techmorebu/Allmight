'use strict';
// ═══════════════════════════════════════════════════════════════════════════════
//  AllMight — ETH/USDC Pool Discovery  v1.0
// ───────────────────────────────────────────────────────────────────────────────
//  PLACEMENT:  scripts/tools/eth_usdc_pool_discovery.js
//  STATUS:     CURRENT — Surface pivot (Boss directive 2026-04-01)
//
//  PURPOSE
//  ───────
//  Discover direct WETH / native USDC pools on Arbitrum across known venues.
...
//
//  REQUIRED ENV VARS (same factory set as other discovery scripts — already set)
//    ARB_UNISWAP_V3_FACTORY   = 0x1F98431c8aD98523631AE4a59f267346ea31F984
//    ARB_CAMELOT_V3_FACTORY   = 0x1a3c9B1d2F0529D97f2afC5136Cc23e58f1FD35B  (Algebra poolByPair, confirmed)
//    ARB_SUSHISWAP_V3_FACTORY = 0x1af415a1EbA07a4986a52B6f2e7dE7003D82231e  (skip_venue_unresolved)
//    ARB_RAMSES_V2_FACTORY    = 0xa67f82621540017a679153423CA0B8a1b4518B49  (skip_venue_unresolved)
//
//  USAGE
//  ─────
//  node -r dotenv/config scripts/tools/eth_usdc_pool_discovery.js
//  node -r dotenv/config scripts/tools/eth_usdc_pool_discovery.js --json
//  node -r dotenv/config scripts/tools/eth_usdc_pool_discovery.js --verbose
// ═══════════════════════════════════════════════════════════════════════════════
...
const ADDR = {
  WETH        : TOKENS.WETH.address.toLowerCase(),
...
const VENUES = [
  ...
  },
  {
    ...
    slotFn : 'slot0',
    // Status: skip_venue_unresolved — all fee-tier getPool() calls failed in prior discovery runs
  },
  {
    venue  : 'ramses_v2',
    type   : 'v3_factory',
...
async function inspectPool(rpc, venue, poolAddress, feeTierOverride, blockNumber, verbose) {
  ...
  const isDirectPair = (tok0 === 'WETH' && tok1 === 'USDC_NATIVE') ||
                       (tok0 === 'USDC_NATIVE' && tok1 === 'WETH');
  if (!isDirectPair) {
    return { ...base, feeTierQueried: feeTierOverride, token0: t0, token1: t1,
             token0Symbol: tok0, token1Symbol: tok1, liquidityRaw,
  ...
  // Indicative price (WETH in USDC terms)
  let indicativePrice = null, depthUsd = null;
  if (sqrtPriceX96) {
    try {
      const sqrtP = Number(sqrtPriceX96) / (2 ** 96);
      const dec0  = tok0 === 'WETH' ? 18 : 6;
      const dec1  = tok0 === 'WETH' ? 6  : 18;
      const raw   = sqrtP * sqrtP * Math.pow(10, dec0 - dec1);
      indicativePrice = tok0 === 'WETH' ? raw : 1 / raw;  // always WETH in USDC
      depthUsd = activeTickUsd(liquidityRaw, sqrtPriceX96, dec0, dec1);
      if (tok0 !== 'WETH') depthUsd = activeTickUsd(liquidityRaw, sqrtPriceX96, 6, 18);
    } catch {}
  }
  ...
  if (venue.type === 'algebra_factory') {
    ...
    try {
      const { result } = await rpc.callDetailed(
        ...
        async (provider) => {
          ...
          return factory.poolByPair(TOKENS.WETH.address, TOKENS.USDC_NATIVE.address, { blockTag: blockNumber });
        },
        { timeoutMs: 5000 }
      );
      poolAddress = result;
    ...
    if (!poolAddress || poolAddress === ZERO_ADDR) {
      results.rejected.push({ venue: venue.venue, status: 'reject_zero_address',
        rejectReason: 'no pool for ETH/USDC on this algebra factory' });
      return results;
    }
    ...
    try {
      const { result } = await rpc.callDetailed(
        ...
        async (provider) => {
          ...
          return factory.getPool(TOKENS.WETH.address, TOKENS.USDC_NATIVE.address, feeTier, { blockTag: blockNumber });
        },
        { timeoutMs: 5000 }
      );
      poolAddress = result;
...
function printReport(output) {
  ...
  console.log('\n' + DBLE);
  console.log(` ALLMIGHT — ETH/USDC POOL DISCOVERY`);
  ...
  if (output.kept.length === 0) {
    console.log(' No direct ETH/native-USDC pools found with live liquidity.');
  } else {
...
async function main() {
  ...
  if (useVerbose) console.log(`[disc] block ${blockNumber}  pair: ETH/native-USDC  venues: ${VENUES.length}`);

  const output = {
    ts: nowIso(), chain: CHAIN, pair: 'ETH/USDC', blockNumber,
