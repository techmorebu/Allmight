...
def build_risk_penalty_csv(grid_assets: List[str], regime_state_json: Path, out_csv: Path) -> dict:
    ...
    rp = float(state.get("risk_penalty", 0.0))
    rows = [
        ComponentRow(asset=a, value=rp, audit={"source": str(regime_state_json), "risk_penalty": rp})
        for a in grid_assets
    ]
    write_component_csv(out_csv, rows)
    return {"out_csv": str(out_csv), "risk_penalty": rp}
