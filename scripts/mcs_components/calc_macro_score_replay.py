...
def build_macro_score_csv(grid_assets: List[str], regime_state_json: Path, out_csv: Path) -> dict:
    ...
    macro_score = float(d.get("macro_score", 0.0) or 0.0)
    ...
    for a in grid_assets:
        rows.append(
            ComponentRow(
                asset=a,
                value=macro_score,
                audit={"macro_score": macro_score, "source": str(regime_state_json)},
            )
        )

    write_component_csv(out_csv, rows)
    return {"out_csv": str(out_csv), "macro_score": macro_score}
