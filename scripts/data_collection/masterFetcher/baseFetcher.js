...
const FETCH_CONCURRENCY = Math.max(
  1,
  Number(process.env.BASE_FETCHER_CONCURRENCY || 4)
);
...
async function baseFetcher() {
    ...
    return {
      ...
      data: {
        ...
        stats: {
          ...
          successCount: 0,
          failureCount: UNISWAP_V3_POOLS.length + AERODROME_POOLS.length,
          uniswapV3: {
  ...
  const combined = [...uniResults, ...aeroResults];
