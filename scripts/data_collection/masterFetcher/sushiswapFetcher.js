...
async function erc20Decimals(address) {
  ...
  const out = await rpc.call('erc20.decimals', async (provider) => {
...
async function fetchPoolData(poolConfig) {
  try {
    const reserves = await rpc.call(`sushi.getReserves:${poolConfig.pair}`, async (provider) => {
    ...
    const [dec0, dec1] = await Promise.all([
      erc20Decimals(poolConfig.token0),
      erc20Decimals(poolConfig.token1),
    ]);
