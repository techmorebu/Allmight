...
def _make_broker(*, net: NetworkGate):
    decl = AdapterDeclaration(
        adapter_id="phase10_http_snapshot_kraken",
        version="phase10_v0",
    ...
    return AdapterBroker(
        ...
        declarations={"phase10_http_snapshot_kraken": decl},
    )


def test_kraken_snapshot_denies_when_network_disabled(monkeypatch):
    called = {"n": 0}
    def boom(*a, **k):
        called["n"] += 1
    ...
    with pytest.raises(Exception) as e:
        broker.call(
            ...
            params={"pair": "BTC-USD", "domain": "api.kraken.com"},
        )
...
def test_kraken_snapshot_allows_allowlisted_domain_and_calls_urlopen_once(monkeypatch):
    # Kraken v0 format (simplified): {"error": [], "result": {"XXBTZUSD": {"c": ["123.45", "1.0"]}}}
    payload = json.dumps({"error": [], "result": {"XXBTZUSD": {"c": ["123.45", "1.0"]}}}).encode("utf-8")
    ...
    assert out["pair"] in ("BTC-USD", "XXBTZUSD")
    assert float(out["price"]) == 123.45
    assert out["source"]
    assert called["n"] == 1


def test_kraken_snapshot_denies_too_large_response(monkeypatch):
    big = b"x" * (262144 + 2)
...
def test_kraken_snapshot_denies_bad_json(monkeypatch):
    def fake_urlopen(req, timeout):
...
def test_kraken_snapshot_denies_missing_price(monkeypatch):
    payload = json.dumps({"error": [], "result": {"XXBTZUSD": {"c": []}}}).encode("utf-8")
    ...
    assert "DENY_SCHEMA" in str(e.value)
