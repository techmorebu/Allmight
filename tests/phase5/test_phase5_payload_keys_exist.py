...
def test_phase5_payload_keys_exist(tmp_path: Path) -> None:
    phase4 = {
        "phase": 4,
        "asof": "i60",
        "grid": "GRID_TEST",
        ...
        "assets": {
            "BTC": {
                ...
                "overrides_applied": [],
                "permissions": {"allow_arbitrage": True, "allow_directional": True, "allow_flashloan": True},
    ...
    it = doc["intents"][0]
    assert "intent_payload" in it
    assert set(it["intent_payload"].keys()) == {"arbitrage", "directional", "flashloan"}
    for k in ("arbitrage", "directional", "flashloan"):
        assert "enabled" in it["intent_payload"][k]
