#!/usr/bin/env python3
"""
Patch writer: fixes both Redis adapters for real payload structure.

Fixes:
  1. status path: payload["data"]["status"] not payload["status"]
  2. prices path: payload["data"]["data"]["prices"] (confirmed correct)
  3. fee normalization: fee is decimal (0.05, 0.3) → multiply by 100 to get bps
  4. Uniswap: capture liquidity + tick for pool_state
  5. Sushiswap: reserves confirmed present, capture reserveUSD as tvl_usd fallback

Run from project root: python3 write_patch_adapters_v2.py
"""
import os
...
# ─────────────────────────────────────────────
files = {
    "uniswap_v3.py": UNISWAP,
    "sushiswap_v2.py": SUSHISWAP,
}
...
print("\nDone. Run:")
print("  python3 scripts/run_reality_check.py --single --tier 1000")
