...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def execute_alert(self, chain, pair, gross_bps, net_usd,
        ...
        text = (
            ...
            f" Total P&L:  ${sess.get('total_pnl',0):.4f}  [session]\n"
            f"```"
        )
    ...
    def shadow_report(self, **kwargs):
        ...
        text = (
            ...
            f"\n"
            f"{'─'*44}\n"
            f" ALL-TIME SHADOW (simulated)\n"
            f"{'─'*44}\n"
            f" Trades:    {m.get('shadow_alltime',at).get('executed',0):>5} exec"
            f" / {m.get('shadow_alltime',at).get('skipped',0):>4} skip\n"
            f" Hit rate:  {m.get('shadow_alltime',at).get('hit_rate',0):>5.1f}%"
            f"  Win rate: {m.get('shadow_alltime',at).get('win_rate',0):>5.1f}%\n"
            f" Total P&L: ${m.get('shadow_alltime',at).get('total_pnl',0):>9.4f}  [SIMULATED]\n"
            f" Best:      ${m.get('shadow_alltime',at).get('best_trade',0):>9.4f}\n"
            f"{'─'*44}\n"
            f" ALL-TIME LIVE (on-chain)\n"
            f"{'─'*44}\n"
            f" Trades:    {m.get('live_alltime',{}).get('executed',0):>5} exec\n"
            f" Hit rate:  {m.get('live_alltime',{}).get('hit_rate',0):>5.1f}%"
            f"  Win rate: {m.get('live_alltime',{}).get('win_rate',0):>5.1f}%\n"
            f" Total P&L: ${m.get('live_alltime',{}).get('total_pnl',0):>9.4f}  [ON-CHAIN]\n"
            f" Best:      ${m.get('live_alltime',{}).get('best_trade',0):>9.4f}\n"
            f"\n"
            f"{'─'*44}\n"
